<template>
  <div class="edit-job">
    ...
    <form @submit.prevent="submit">
      ...
      <h3 class="group-title">EMPLOYEUR</h3>
      <WedoInput
      ...
      <h3 class="group-title">NIVEAUX LINGUISTIQUES REQUIS</h3>
      <WedoSelect
      ...
      <h3 class="group-title">
        POUR L'ADEM DANS LE CAS OU LE POSTE N'A PAS ENCORE ÉTÉ DÉCLARÉ
        (FACULTATIF)
      </h3>
      <WedoInput
    ...
    async init() {
      try {
        this.job = (await this.$axios.$get(
          '/joboffer/id/' + this.$route.query.id
        )).jobOffer
        console.log(this.job)

        this.position = {
          lat: this.job.map_latitude,
          lng: this.job.map_longitude,
          street_number: this.job.houseNbr,
          route: this.job.street,
          locality: this.job.city,
          administrative_area_level_1: '',
          country: '',
          postal_code: this.job.zip_code
        }
        ...
        const jobRegionIds = JSON.parse(this.job.region)
        regions.forEach(region => {
          this.regions.push({ value: region.region_id, text: region.name })
          jobRegionIds.forEach(regionId => {
    ...
    submit() {
      ...
      this.$validator.validateAll().then(async () => {
        ...
        if (!this.errors.any()) {
          try {
            const job = {
              listing_id: this.job.listing_id,
